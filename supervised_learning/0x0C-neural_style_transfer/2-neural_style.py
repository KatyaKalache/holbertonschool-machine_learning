    ...
    def load_model(self):
        ...
        for layer in vgg.layers:
            ...
            if isinstance(layer, tf.keras.layers.MaxPooling2D):
                ...
                avg.add(layer)
            avg.add(layer)
        style_outputs = [avg.get_layer(layer).get_output_at(1) for
                         layer in self.style_layers]
        content_outputs = [avg.get_layer(layer).get_output_at(1) for
                           layer in self.content_layer]
